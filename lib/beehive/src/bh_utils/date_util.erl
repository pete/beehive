...
% A standard Unix timestamp; use this when talking to the outside world.
epoch() ->
  now_to_seconds(now()).
...
is_time_sooner_than({Date, Time}, Mark) ->
    is_time_sooner_than(
      calendar:datetime_to_gregorian_seconds({Date, Time}), Mark);
is_time_sooner_than(Time, {DateMark, TimeMark}) ->
    is_time_sooner_than(
      Time, calendar:datetime_to_gregorian_seconds({DateMark, TimeMark}));
...
% Adds a zero to the beginning of a string if there is only one character in it.
