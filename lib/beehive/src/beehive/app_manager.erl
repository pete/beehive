...
%%====================================================================
request_to_expand_app(App) ->
  gen_server:cast(?SERVER, {request_to_expand_app, App}).
...
request_to_start_new_bee_by_name(Name) ->
  request_to_start_new_bee_by_name(Name, undefined).
...
terminate_bee(Bee, Caller) ->
  gen_server:call(?SERVER, {terminate_bee, Bee, Caller}, infinity).

request_to_update_app(App) ->
  gen_server:cast(?SERVER, {request_to_update_app, App}).

request_to_save_app(App) ->
  gen_server:call(?SERVER, {request_to_save_app, App}).
...
init([]) ->
  ...
  ScratchDisk =
    config:search_for_application_value(scratch_dir, ?BEEHIVE_DIR("tmp")),
  RunDir =
    config:search_for_application_value(squashed_storage, ?BEEHIVE_DIR("run")),
...
handle_call({update_application, Name, ConfigProplist}, From, State) ->
  handle_queued_call(fun() ->
                         internal_update_application(Name, ConfigProplist) end,
                     From, State);
...
handle_info({'EXIT',_Pid,
             {received_unknown_message,
              {_FsmState, {error, {babysitter, Msg}}}}},
            State) ->
...
handle_info({bee_updated_normally,
...
internal_update_application(Name, ConfigProplist) ->
  case apps:find_by_name(Name) of
    App when is_record(App, app) ->
      internal_update_application(App, ConfigProplist);
    _ -> {error, app_not_found}
  end.
...
%% If the instance is not busy, the timeout has been exceeded and there
%% are other application instances running
    ...
    #bee{pid = Pid} = Bee when is_record(Bee, bee) ->
      case is_pid(Pid) andalso Bee#bee.os_pid =/= undefined of
        true ->
          ...
          ?LOG(debug, "Started app_killer_fsm for ~p: ~p",
               [Bee#bee.app_name, P]),
          P;
        _ -> ok
      end;
